...
export function SponsoredAccessClient({ userName, code, pass }: { userName: string; code: string; pass: PassView }) {
  ...
  return <main className="claim-shell">
    ...
    <section className="claim-card">
      <div className="claim-art"><span><Gift /></span><img src="/capi-welcome.png" alt="Capi Coach welcoming a sponsored learner" /></div>
      ...
      : invalid ? <div className="claim-copy invalid"><span className="claim-kicker"><XCircle /> PASS UNAVAILABLE</span><h1>{pass?.status === "expired" ? "This study pass has expired." : pass?.status === "claimed" ? "This pass has already been claimed." : "This pass link is not valid."}</h1><p>A Capi-Helper gift can be claimed once by one student. Ask the sender for a new link if you think this is a mistake.</p><Link href="/dashboard">Go to dashboard <ArrowRight /></Link></div>
