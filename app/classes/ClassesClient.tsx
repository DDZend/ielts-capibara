...
export function ClassesClient({ userName, initialSnapshot }: { userName: string; initialSnapshot: StudentClassSnapshot }) {
  ...
  return <main className="classes-shell">
    ...
    <section className="classes-hero"><div><span><Sparkles /> CLASSES & HOMEWORK</span><h1>{firstName}, your teacher support is organised here.</h1><p>Book the meetings included in your membership, reschedule responsibly, join lessons and keep teacher-assigned work in one clear timeline.</p><div><BadgeCheck /><span><small>{billingPlanLabel(snapshot.allowance.planInterval ?? "").toUpperCase()}</small><b>{snapshot.allowance.label}</b></span></div></div><aside><img src="/capi-plan.png" alt="Capy Coach organising an IELTS study schedule" /><strong>{snapshot.allowance.remainingThisWeek}<small>meetings remaining this week</small></strong></aside></section>
