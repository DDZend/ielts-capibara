...
export function CapiCoachSupportClient({ userName, initialEscalations }: { userName: string; initialEscalations: TutorEscalationView[] }) {
  ...
  const reply = async (id: number) => {
    ...
    if (response?.ok && data?.escalations) {
      setItems(data.escalations); setReplies((current) => ({ ...current, [id]: "" })); setMessage("Your reply is now visible in the student’s Capi Coach conversation.");
    } else setError(data?.error ?? "The reply could not be sent.");
    setBusy(null);
  };

  return <main className="tutor-support-shell">
    <nav className="creator-topbar"><Link href="/creator"><ArrowLeft /> Creator Studio</Link><span><BotMessageSquare /> Capi Coach support</span><div><small>Teacher</small><b>{userName}</b></div></nav>
    <header className="tutor-support-hero"><div><span><ShieldCheck /> HUMAN-IN-THE-LOOP TUTORING</span><h1>Help Capi answer responsibly.</h1><p>Capi sends uncertain or sensitive questions here instead of inventing an answer. Your reply goes straight into the student’s saved tutor conversation.</p></div><aside><strong>{pending.length}<small>waiting for a teacher</small></strong><strong>{resolved.length}<small>resolved conversations</small></strong><button onClick={() => void refresh()} disabled={busy === "refresh"}>{busy === "refresh" ? <LoaderCircle className="spin" /> : <RefreshCw />} Refresh queue</button></aside></header>
    <div className="tutor-support-content">{message && <p className="creator-message"><Check /> {message}</p>}{error && <p className="creator-error">{error}</p>}
      <section><header><div><small>NEEDS REVIEW</small><h2>Student questions</h2></div><span>{pending.length} open</span></header><div className="tutor-support-list">{pending.map((item) => <article key={item.id}><header><div><b>{item.userEmail}</b><small><Clock3 /> {formatDate(item.createdAt)}</small></div><span>Waiting</span></header><blockquote>{item.question}</blockquote><p><b>Why Capi escalated:</b> {item.reason}</p><label><span>Your answer to the student</span><textarea maxLength={1500} value={replies[item.id] ?? ""} onChange={(event) => setReplies((current) => ({ ...current, [item.id]: event.target.value }))} placeholder="Give a clear explanation, confirm the rule, or point the student to the right lesson…" /></label><button disabled={busy !== null || !(replies[item.id] ?? "").trim()} onClick={() => void reply(item.id)}>{busy === item.id ? <LoaderCircle className="spin" /> : <MessageSquareReply />} Send teacher reply</button></article>)}{!pending.length && <div className="tutor-support-empty"><Check /><b>Everything is answered</b><p>Capi has no questions waiting for teacher review.</p></div>}</div></section>
