    ...
    <main className="dashboard-shell">
      <aside className={`dashboard-sidebar ${sidebarOpen ? "open" : ""}`}>
        ...
        <nav aria-label="Dashboard navigation">
          <button className="nav-item active"><span><LayoutDashboard /></span>Dashboard</button>
          <Link className="nav-item prominent" href="/assessment"><span><Zap /></span>Take assessment<ChevronRight /></Link>
          <p>LEARN</p>
          {modules.map(({ skill, icon: Icon, className }) => <button key={skill} className="nav-item"><span className={className}><Icon /></span>{skill}<ChevronRight /></button>)}
          <div className="nav-line" />
          <button className="nav-item"><span><BarChart3 /></span>My progress</button>
          <button className="nav-item"><span><Video /></span>Live classes<span className="nav-badge">2</span></button>
          <button className="nav-item"><span><CalendarDays /></span>Study calendar</button>
          <div className="nav-line" />
          <button className="nav-item"><span><Settings /></span>Settings</button>
        </nav>
        <button className="sidebar-help"><HelpCircle /><span><b>Need help?</b><small>Ask Capi Coach</small></span><ChevronRight /></button>
      </aside>
      ...
      <div className="dashboard-main">
        <header className="dashboard-topbar">
          ...
          <div className="topbar-actions">
            ...
            <button className="notification" aria-label="Notifications"><Bell /><i>2</i></button>
          ...
          <div className="dashboard-columns">
            <div className="dashboard-primary">
              <section className="progress-section dashboard-card">
                ...
                <div className="skill-grid">{modules.map(({ skill, icon: Icon, className, tasks }) => <article className={`skill-card ${className}`} key={skill}><span><Icon /></span><div><small>{skill}</small><b>{bands[skill]?.toFixed(1) ?? "—"}</b></div><p>{latest ? tasks : "Complete assessment"}</p><ChevronRight /></article>)}</div>
              </section>
              ...
              <section className="recent-card dashboard-card"><div className="card-heading"><h2>Recent activity</h2><button>View all</button></div><div className="activity-list">{latest && <article><span className="activity-icon assessment"><BarChart3 /></span><div><b>Assessment result saved</b><small>Overall {latest.overallBand.toFixed(1)} · priority: {latest.prioritySkill}</small></div><time>{new Intl.DateTimeFormat("en", { day: "numeric", month: "short" }).format(new Date(latest.createdAt))}</time></article>}<article><span className="activity-icon reading"><BookOpen /></span><div><b>Matching headings practice</b><small>6 of 8 correct · Reading</small></div><time>Yesterday</time></article><article><span className="activity-icon speaking"><Mic2 /></span><div><b>Speaking warm-up completed</b><small>Fluency and topic vocabulary</small></div><time>16 Jul</time></article></div></section>
            </div>

            <aside className="dashboard-secondary">
              <section className="calendar-card dashboard-card"><div className="card-heading"><div><span className="eyebrow">July 2026</span><h2>Your study week</h2></div><button aria-label="Open full calendar"><CalendarDays /></button></div><div className="week-labels">{["M", "T", "W", "T", "F", "S", "S"].map((day, index) => <span key={`${day}-${index}`}>{day}</span>)}</div><div className="week-days">{monthDays.map(({ day, state }) => <button key={day} className={state}>{day}{state === "done" && <Check />}</button>)}</div><div className="calendar-legend"><span><i className="done" /> Studied</span><span><i className="planned" /> Planned</span></div></section>
              ...
              <section className="live-class-card dashboard-card"><div className="live-image"><span>LIVE CLASS</span><img src="/capi-headset.png" alt="Capi Coach wearing a coaching headset" /></div><div><span className="eyebrow">Tuesday · 18:30</span><h3>Speaking Part 2: confident long turns</h3><p><Video /> With Anna Müller · 45 min</p><button className="button soft">Reserve my place <ArrowRight /></button></div></section>
            </aside>
          </div>
        </div>
      </div>
      ...
      {chatOpen && <section className="capi-chat" aria-label="Capi Coach chat"><header><img src="/capi-profile.png" alt="" /><span><b>Capi Coach</b><small><i /> Ready to help</small></span><button onClick={() => setChatOpen(false)} aria-label="Close chat"><X /></button></header><div className="chat-body"><p>Hi {firstName}! I&apos;m focusing your plan on {latest?.prioritySkill.toLowerCase() ?? "finding your starting level"}. What would you like help with?</p><div className="chat-suggestions"><button>Explain today&apos;s lesson</button><button>How do I reach Band {targetBand.toFixed(1)}?</button><button>Give me a 5-minute warm-up</button></div></div><form onSubmit={(event) => event.preventDefault()}><input aria-label="Message Capi Coach" placeholder="Ask about your study plan…" /><button aria-label="Send message"><Send /></button></form></section>}
    </main>
  );
}
