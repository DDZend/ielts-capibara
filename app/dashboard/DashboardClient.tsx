    ...
    <main className="dashboard-shell">
      <aside className={`dashboard-sidebar ${sidebarOpen ? "open" : ""}`}>
        ...
        <nav aria-label="Dashboard navigation">
          ...
          <p>LEARN</p>
          {modules.map(({ skill, icon: Icon, className }) => <Link key={skill} href={skill === "Speaking" ? "/speaking" : skill === "Writing" ? "/writing" : skill === "Reading" ? "/reading" : "#modules"} onClick={() => setSidebarOpen(false)} className="nav-item"><span className={className}><Icon /></span>{skill}<ChevronRight /></Link>)}
          ...
          <div className="dashboard-columns">
            <div className="dashboard-primary">
              <section className="progress-section dashboard-card" id="progress">
                ...
                <div className="skill-grid" id="modules">{modules.map(({ skill, icon: Icon, className, tasks }) => <Link className={`skill-card ${className}`} href={skill === "Speaking" ? "/speaking" : skill === "Writing" ? "/writing" : skill === "Reading" ? "/reading" : "#today-plan"} aria-label={`Practise ${skill}. ${bands[skill] ? `Current estimate ${bands[skill]?.toFixed(1)}` : "Complete the assessment first"}`} key={skill}><span className="skill-card-icon"><Icon /></span><div><small>{skill}</small><b>{bands[skill]?.toFixed(1) ?? "—"}</b></div><p>{latest ? tasks : "Complete assessment"}</p><ChevronRight className="skill-card-chevron" /></Link>)}</div>
              </section>
