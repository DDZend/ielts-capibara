...
export function BillingClient({ userName, summary, checkoutConfigured, paywallActive, accessRequired, checkoutResult }: BillingClientProps) {
  ...
  return <main className="billing-shell">
    ...
    <section className="billing-hero"><div><span className="billing-kicker"><Sparkles /> Membership & sponsored access</span><h1>Choose your path.<br /><em>Keep every achievement.</em></h1><p>{firstName}, start with seven days of platform access or choose self-study, guided group learning, or personal teacher coaching.</p><div className="billing-access-chip"><BadgeCheck /><span><small>Your access</small><b>{activeAccessLabel}</b></span></div></div><div className="billing-hero-art"><span><Star fill="currentColor" /><b>{summary.earnedCoins.toLocaleString()}</b><small>lifetime-earned coins</small></span><img src="/capi-plan.png" alt="Capy Coach planning a study membership" /></div></section>
    ...
    <div className="billing-page">
      ...
      <section className="billing-status-grid">
        ...
        <article><span><Clock3 /></span><div><small>Pass access</small><h2>{summary.starterPass ? "7-Day Starter active" : summary.activePass ? "24-hour gift active" : "No active pass"}</h2><p>{summary.starterPass ? `Available until ${new Intl.DateTimeFormat("en", { dateStyle: "medium", timeStyle: "short" }).format(new Date(summary.starterPass.expiresAt))}` : summary.activePass ? `Available until ${new Intl.DateTimeFormat("en", { dateStyle: "medium", timeStyle: "short" }).format(new Date(summary.activePass.expiresAt))}` : "Buy a Starter Pass or claim a Capy-Helper gift."}</p></div></article>
        <article><span><Star fill="currentColor" /></span><div><small>Your checkout discount</small><h2>{summary.discountPercent ? `${summary.discountPercent}% unlocked` : "Keep earning"}</h2><p>Based on {summary.earnedCoins.toLocaleString()} lifetime-earned Capy-Coins. Giving a pass does not remove your discount.</p></div></article>
      </section>

      <section className="billing-plans package-pricing">
        <header><div><span>MEMBERSHIP OPTIONS</span><h2>Start independently. Add support when you need it.</h2><p>New memberships activate immediately after Stripe confirms payment. Higher-tier upgrades automatically credit unused plan value.</p></div><span className="billing-discount-badge"><Star fill="currentColor" /> {summary.discountPercent ? `${summary.discountPercent}% Capy discount` : "Discounts from 500 coins"}</span></header>
        <div className="billing-promo-box"><BadgePercent /><div><b>Have a promotion code?</b><small>It reduces the first payment while your Capy-Coin discount remains automatic.</small></div><input value={promoInput} onChange={(event) => { setPromoInput(event.target.value.toUpperCase()); setPromotion(null); }} maxLength={32} placeholder="ENTER CODE" /><button disabled={!promoInput.trim() || busy !== null || Boolean(activeSubscription)} onClick={() => void applyPromotion()}>{busy === "promo" ? "Checking…" : promotion ? <><Check /> Applied</> : "Apply code"}</button></div>
