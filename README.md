...
Signed-in students receive a persistent three-task daily plan at `/dashboard`. Task completions drive Capy-Coins, streaks, weekly progress and recent activity. The protected `/mock-test` route provides a four-skill weekend practice mock, saves one calculated result per week and compares it with the previous attempt. Writing responses and microphone recordings are never persisted.
